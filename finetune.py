...
from model import MyModel, MyModelForFinetune
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", default="train-EXP-20230502-214308/199-model.pt")
    ...
    parser.add_argument('--lr', default=3e-6, type=float)  # 3e-6 # 3e-4
    ...
    model = MyModelForFinetune(model=pretrained, num_variables=train_set.data.shape[-1],
                               num_features=args.num_features, window_length=args.window_length)
    model.cuda()
    ...
    torch.save(model.state_dict(), os.path.join(args.save, "init-model.pt"))
    ...
    for epoch in range(args.num_epochs):
        ...
        torch.save(model.state_dict(), os.path.join(args.save, "{}-model.pt".format(epoch + 1)))
